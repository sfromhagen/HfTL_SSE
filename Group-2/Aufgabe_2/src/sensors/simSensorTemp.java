...
public class simSensorTemp {
	...
	public static void main(String[] args) throws Exception {
		...
		OptionSpec<Integer> oPort = parser.accepts("p").withOptionalArg().ofType(Integer.class).describedAs("port")
				.defaultsTo(4242);
		OptionSpec<InetAddress> oInterface = parser.accepts("i").withOptionalArg()
				.withValuesConvertedBy(new InetAddressConverter());
		OptionSpec<String> oSerial = parser.accepts("s").withOptionalArg().ofType(String.class).describedAs("serial")
				.defaultsTo("1234567890");
		OptionSpec<Integer> oDebugLevel = parser.accepts("d").withOptionalArg().ofType(Integer.class)
		...
		if (options.has(oHelp)) {
			// User asked for help.
			// Dump parameter text and stop execution (as he needs to fix the
			// arguments)
		...
		// jopt does not support default values for InetAddress for some reason.
		// Workaround:
